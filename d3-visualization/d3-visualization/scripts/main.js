d3.json('data/linechartdata.json').then(data => {
  ...
  const yScale = drawLinearAxis(0, d3.max(data, d => d.distance), graphHeight, 0, graph)
  const xScale = drawBandAxis(data.map(d => d.year), 0, graphWidth, graph, graphHeight)

  drawBar(xScale, yScale, graph, data, 'year', 'distance', graphHeight, 50)
});
