var script = (function (exports) {
  ...
  const drawColumn = (columnArg) => {
   ...
   const yScale = drawLinearAxis({
    domainMin: 0,
    domainMax: max(columnArg.data, d => d.value),
  ...
  const drawLine = (lineArg) => {
    const axisObj ={
      domainMin: 0,
      domainMax: max(lineArg.data, d => d.value),
  ...
  const drawPie = (pieArg) =>{
      ...
      const pie$1 = pie()(pieArg.data.map(d => d.value));
  ...
  const drawBar = (barArg) => {
    ...
    const xScale = drawBarLinearAxis({
        parentGroup: barArg.parentGroup,
        domainMin: 0,
        domainMax: max(barArg.data, d => d.value), 
        rangeMin: 0,
